...
function getOptions(options) {
  ...
  if (importer) {
    options.importer = function(file, prev, key) {
      console.log('here', file, prev, key);

      var done = function(data) {
        binding.importedCallback2({
          index: key,
          objectLiteral: data
        });
      };
...
module.exports.info = function() {
  ...
  return [
           'fis-sass version: ' + package.version,
           'libsass version: ' + package.libsass
         ].join('\n');
};

// 暂只支持options.data用法，因为fis中只会这么用。
module.exports.sass2scss = function( options ) {
    return binding.sass2scss(options);
}
